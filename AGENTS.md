...
**Project Name**: Praneet Panmai LINE App

**Repository**: https://github.com/mojisejr/praneet-panmai-line-app
**Author**: mojisejr

**Description**: A mobile-first LINE OA integrated web application designed for durian plant sales with tiered pricing calculator. Features include plant catalog management, member authentication via LINE LIFF, and quantity-based pricing with role-based access control.
...
- **Web Framework**: Next.js 14.2.15 with App Router
- **Database**: PostgreSQL (via Supabase)
- **Database Migrations**: Raw SQL with custom migration system
- **Authentication**: Custom LINE LIFF integration with JWT bridge to Supabase
- **Frontend**: React with shadcn-ui + Tailwind CSS v3.4.16
...
├── src/pages/api/              # Legacy API routes (LINE session bridge)
├── lib/supabase/migrations/    # SQL database migrations
...
```
# Core tables for durian plant calculator system
members (id, line_user_id, display_name, role, contact_info, registration_date, is_active, created_at, updated_at)
products (id, variety_name, size, plant_shape, base_price, is_available_in_store, image_url, description, is_active, created_at, updated_at)
price_tiers (id, product_id, min_quantity, max_quantity, special_price, is_active, created_at, updated_at)
```

### Key Features

- **Plant Catalog Management**: Complete durian plant profiles with variety names, sizes, and shapes
- **Tiered Pricing Calculator**: Quantity-based pricing with discount tiers for bulk purchases
- **LINE OA Integration**: LIFF-based authentication and seamless LINE platform experience
- **Member Management**: LINE user registration with role-based access (member/admin)
- **Mobile-First Design**: Optimized for LINE in-app browser and mobile devices
- **Thai Language Support**: Native Thai interface with localized plant variety names
- **Real-time Pricing**: Dynamic calculation based on quantity tiers and product availability
- **Image Storage**: Plant photos with Supabase Storage and RLS policies
- **QR Code Integration**: Member scanning and quick access features
...
- **LIFF Initialization**: < 2 seconds for LINE SDK loading
...
- **Tests**: Component and service tests with proper test setup
- **Type Safety**: Full TypeScript coverage with proper interfaces
- **Error Handling**: Comprehensive error boundaries and LINE LIFF error handling
...
- **LINE Token Security**: JWT-based session bridge with LINE ID token verification
- **Input Validation**: Validate all user inputs with zod schemas
- **Session Management**: Custom JWT tokens with 24-hour expiration
- **HTTPS Only**: Enforced in production via Vercel
- **Row-Level Security**: Zero-trust data access via Supabase RLS
- **LINE Platform Compliance**: LIFF SDK security best practices
...
- **Database Access**: All queries use Supabase client with parameterization
- **Authentication**: Custom LINE LIFF + JWT bridge implementation
- **LINE Security**: LIFF SDK proper initialization and token validation
...
- **LINE Platform**: Compliance with LINE OA and LIFF security guidelines
